<%-- 
    Document   : Ordenes
    Created on : 09-jun-2022, 0:27:46
    Author     : 1Usuario
--%>

<%@page import="ModeloDAO.ProductoDAO"%>
<%@page import="ModeloVO.ProductoVO"%>
<%@page import="ModeloDAO.UsuarioDAO"%>
<%@page import="ModeloVO.UsuarioVO"%>
<%@page import="java.util.ArrayList"%>
<%@page import="ModeloDAO.OrdenDetallesDAO"%>
<%@page import="ModeloVO.OrdenDetallesVO"%>
<%@page import="ModeloVO.DetallesProductoVO"%>
<%@page import="ModeloDAO.DetallesProductoDAO"%>
<%@page import="ModeloVO.OrdenesVO"%>
<%@page import="ModeloDAO.OrdenesDAO"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>

<%
    HttpSession buscarSesion = (HttpSession) request.getSession();
    UsuarioVO usuVO1 = null;
    usuVO1 = (UsuarioVO) buscarSesion.getAttribute("datosUsuario");
%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"
              integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"
                integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p"
        crossorigin="anonymous"></script>
        <link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/1.12.1/css/jquery.dataTables.css">

        <link rel="stylesheet" href="css/estilos.css">
        <title>JSP Page</title>
    </head>
    <body>

        <div class="col-md-10 mx-auto ">
            <h1 class="text-center mb-5 ">Orden de Produccion</h1>
            <div class="d-flex"> 
                <div>
                    <button id="btnAñadirOrden"  type="button" class="boton btn" data-bs-toggle="modal" data-bs-target="#modalAñadirOrden" >Registrar
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus-circle" viewBox="0 0 16 16">
                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                        <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
                        </svg>
                    </button>
                </div>
                <div class="ms-2 me-2">
                    <button id="btnAñadirVariasOrdenes" type="button" class="boton btn" data-bs-toggle="modal" data-bs-target="#modalAñadirVariasOrdenes" >Registrar +Ordenes
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus-circle" viewBox="0 0 16 16">
                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                        <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
                        </svg>
                    </button>
                </div>
                <div>
                    <form action="OrdenDetalles" method="post" class="form-group">
                        <button  class="boton btn" title="Reporte de Producto">
                            Generar Reporte
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-download" viewBox="0 0 16 16">
                            <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"/>
                            <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"/>
                            </svg>
                        </button>
                        <input type="hidden" value="10" name="opcion">
                    </form>
                </div>
                <div class="ms-2 me-2">
                    <button id="btnMateriaPrimaPorOrden" class="boton btn" >Materia Prima Por Orden
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-up-right-circle" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M1 8a7 7 0 1 0 14 0A7 7 0 0 0 1 8zm15 0A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.854 10.803a.5.5 0 1 1-.708-.707L9.243 6H6.475a.5.5 0 1 1 0-1h3.975a.5.5 0 0 1 .5.5v3.975a.5.5 0 1 1-1 0V6.707l-4.096 4.096z"/>
                        </svg>
                    </button>
                </div>


            </div>
            <div class="actualizarTBody" id="actualizarTBody">
                <table id="table_id" class="table text-center border-dark ">
                    <thead>
                        <tr class="table-dark">
                            <th>Orden</th>
                            <th>Usuario</th>
                            <th>Producto</th>
                            <th>Cantidad Solicitada</th>
                            <th>Fecha Registro</th>
                            <th>Fecha Entrega</th>
                            <th>Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        <%
                            UsuarioVO usuVO = new UsuarioVO();
                            UsuarioDAO usuDAO = new UsuarioDAO();

                            DetallesProductoVO detProVO = new DetallesProductoVO();
                            DetallesProductoDAO detProDAO = new DetallesProductoDAO();

                            ProductoVO proVO = new ProductoVO();
                            ProductoDAO proDAO = new ProductoDAO();

                            OrdenDetallesVO orddetllVO = new OrdenDetallesVO();
                            OrdenDetallesDAO orddetllDAO = new OrdenDetallesDAO();

                            ArrayList<OrdenDetallesVO> listaOrdenDetalles = orddetllDAO.Listar();
                            for (int i = 0; i < listaOrdenDetalles.size(); i++) {
                                orddetllVO = listaOrdenDetalles.get(i);
                                usuVO = usuDAO.consultaruSUARIO(orddetllVO.getId_Usuarios());
                                detProVO = detProDAO.consultarDetallesProducto(orddetllVO.getId_Detalles_Producto());
                                proVO = proDAO.consultarProducto(detProVO.getId_Producto());
                        %>
                        <tr>
                            <td><%=orddetllVO.getId_Ordenes()%></td>
                            <td><%=usuVO.getNombre()%></td>
                            <td><%=proVO.getNombre() + " talla-" + detProVO.getTalla()%></td>
                            <td><%=orddetllVO.getCantidadSolicitada()%></td>
                            <td><%=orddetllVO.getFecha_registro()%></td>
                            <td><%=orddetllVO.getFecha_entrega()%></td>
                            <td>
                                <div class="d-flex mx-auto justify-content-center" >
                                    <button data-orden="<%=orddetllVO.getId_Ordenes()%>"
                                            data-idusuario="<%=orddetllVO.getId_Usuarios()%>"
                                            data-unombre="<%=usuVO.getNombre()%>"
                                            data-iddetallesproducto="<%=orddetllVO.getId_Detalles_Producto()%>"
                                            data-nombreproducto="<%=proVO.getNombre()%>"
                                            data-protalla="<%=detProVO.getTalla()%>"
                                            data-canrsolic="<%=orddetllVO.getCantidadSolicitada()%>"
                                            data-fecreg="<%=orddetllVO.getFecha_registro()%>"
                                            data-fecent="<%=orddetllVO.getFecha_entrega()%>"
                                            class="btn boton mt-2  detalles">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-zoom-in" viewBox="0 0 16 16">
                                        <path fill-rule="evenodd" d="M6.5 12a5.5 5.5 0 1 0 0-11 5.5 5.5 0 0 0 0 11zM13 6.5a6.5 6.5 0 1 1-13 0 6.5 6.5 0 0 1 13 0z"/>
                                        <path d="M10.344 11.742c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1 6.538 6.538 0 0 1-1.398 1.4z"/>
                                        <path fill-rule="evenodd" d="M6.5 3a.5.5 0 0 1 .5.5V6h2.5a.5.5 0 0 1 0 1H7v2.5a.5.5 0 0 1-1 0V7H3.5a.5.5 0 0 1 0-1H6V3.5a.5.5 0 0 1 .5-.5z"/>
                                        </svg>
                                    </button>
                                    <button data-orden2="<%=orddetllVO.getId_Ordenes()%>"
                                            data-idusuario2="<%=orddetllVO.getId_Usuarios()%>"
                                            data-unombre2="<%=usuVO.getNombre()%>"
                                            data-iddetallesproducto2="<%=orddetllVO.getId_Detalles_Producto()%>"
                                            data-nombreproducto2="<%=proVO.getNombre()%>"
                                            data-protalla2="<%=detProVO.getTalla()%>"
                                            data-canrsolic2="<%=orddetllVO.getCantidadSolicitada()%>"
                                            data-fecreg2="<%=orddetllVO.getFecha_registro()%>"
                                            data-fecent2="<%=orddetllVO.getFecha_entrega()%>"  class="btn boton ms-3 mt-2 eliminar">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
                                        <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5ZM11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H2.506a.58.58 0 0 0-.01 0H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1h-.995a.59.59 0 0 0-.01 0H11Zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5h9.916Zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47ZM8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5Z"/>
                                        </svg>
                                    </button>
                                </div>
                            </td>
                        </tr>
                        <% }%>
                    </tbody>
                </table>
            </div>
        </div>

        <%--Modal Añadir una sola Orden--%>
        <div class="modal fade" id="modalAñadirOrden" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog ">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="staticBackdropLabel">Añadir Orden</h5>
                        <button type="button" class="btn-close botonCerrar" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="col-md-10 mx-auto justify-content-center">
                            <form class="form-group">
                                <div class="d-flex mx-auto justify-content-center ">
                                    <div class="">
                                        <input type="text" class="form-control" name="txtcantidadSolicitadaReg" id="txtcantidadSolicitadaReg" placeholder="Cantidad">
                                    </div>
                                    <div class="ms-2">
                                        <select name="txtIdDetallesProducto" id="txtIdDetallesProductoReg" class="form-select">
                                            <option>IdDetalles Producto</option>
                                            <%
                                                DetallesProductoDAO dpDAO1 = new DetallesProductoDAO();
                                                for (DetallesProductoVO dpVO1 : dpDAO1.listar()) {
                                                    proVO = proDAO.consultarProducto(dpVO1.getId_Producto());

                                            %>
                                            <option value="<%=dpVO1.getId_Detalles_Producto()%>"><%=proVO.getNombre() + " talla-" + dpVO1.getTalla()%></option>
                                            <%}%>
                                        </select>
                                    </div>       
                                </div>
                                <div class="">
                                    <input type="date" class="form-control mt-2" name="txtFechaEntregaReg" id="txtFechaEntregaReg">
                                </div>
                                <input type="hidden" value="<%=usuVO1.getIdUsuarios()%>" name="txtIdUsuario" id="txtIdUsuarioReg">
                                <input type="hidden" value="1" name="opcionReg" id="opcionReg">
                                <input type="button" id="submitReg" value="Guardar" class="btn boton mt-3">
                                <div id="respuesta"></div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <%--Ventana Modal Añadir varias Ordenes--%>
        <div class="modal fade" id="modalAñadirVariasOrdenes" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="staticBackdropLabel">Añadir Varias Ordenes</h5>
                        <button type="button" class="btn-close botonCerrar" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            <div class="d-flex">
                                <div class="col-md-5 mx-auto">
                                    <form class="form-group">
                                        <div class="d-flex ">
                                            <div class="col-md-3">
                                                <input type="text" class="form-control" name="txtcantidadSolicitadalist" id="txtcantidadSolicitadalist" placeholder="Cantidad">
                                            </div>
                                            <div class="col-md-5 ms-1">
                                                <select name="txtIdDetallesProducto" id="detallesProductolist" class="form-select">
                                                    <option>IdDetalles Producto</option>
                                                    <%
                                                        DetallesProductoDAO dpDAOlistar = new DetallesProductoDAO();
                                                        for (DetallesProductoVO dpVOdpDAOlistar : dpDAOlistar.listar()) {
                                                            proVO = proDAO.consultarProducto(dpVOdpDAOlistar.getId_Producto());
                                                    %>
                                                    <option value="<%=dpVOdpDAOlistar.getId_Detalles_Producto()%>"><%=proVO.getNombre() + " talla-" + dpVOdpDAOlistar.getTalla()%></option>
                                                    <%}%>
                                                </select>
                                            </div>       
                                        </div>
                                        <div class="col-md-8">
                                            <input type="date" class="form-control mt-2" name="txtFechaEntregalist" id="txtFechaEntregalist">
                                        </div>
                                        <input type="hidden" value="<%=usuVO1.getIdUsuarios()%>" name="txtIdUsuario" id="txtIdUsuariolist">
                                        <input type="hidden" value="Jose" name="txtnombreUsuariolist" id="txtnombreUsuariolist">
                                        <input type="hidden" value="5" name="opcionLista" id="opcionLista">
                                        <button id="submitVarios" class="btn boton mt-3" >Añadir</button>
                                    </form>
                                    <div id="respuestaCargaLista"></div>
                                </div>
                                <div class="col-md-7 mx-auto">
                                    <form>
                                        <button class="btn boton" id="cargarLista">Cargar Lista</button>
                                        <input type="hidden" id="opcionCargarLista" value="12">
                                    </form>

                                    <div class="contenerdorLista" id="contenerdorLista">
                                        <table class="table text-center table-bordered border-dark table-hover table-responsive">
                                            <thead>
                                                <tr>
                                                    <th>Num.</th>
                                                    <th>Id Usuario</th>
                                                    <th>Id Detalles Producto</th>
                                                    <th>Cantidad Solicitada</th>
                                                    <th>Fecha Registro</th>
                                                    <th>Fecha Entrega</th>
                                                    <th>Acciones</th>
                                                </tr>
                                            </thead>
                                            <tbody id="tabla">
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <%--Ventana Editar --%>
        <div class="modal fade" id="editarOrdenModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header ">
                        <h5 class="modal-title" id="staticBackdropLabel">Editar Orden <label id="IdOrdenEdit"></label></h5>
                        <button type="button" class="btn-close botonCerrar" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="col-md-10 mx-auto justify-content-center">
                            <form class="form-group">
                                <div class="d-flex mx-auto justify-content-center ">
                                    <div class="">
                                        <input type="text" class="form-control" name="txtcantidadSolicitadaedit" id="txtcantidadSolicitadaEdit" placeholder="Cantidad">
                                    </div>
                                    <div class="ms-2">
                                        <select name="txtIdDetallesProducto" id="txtIdDetallesProductoedit" class="form-select">
                                            <option id="opciontxtIdDetallesProductoedit"></option>
                                            <%
                                                DetallesProductoDAO dpDAOedit = new DetallesProductoDAO();
                                                for (DetallesProductoVO dpVOedit : dpDAOedit.listar()) {
                                                    proVO = proDAO.consultarProducto(detProVO.getId_Producto());
                                            %>
                                            <option value="<%=dpVOedit.getId_Detalles_Producto()%>"><%=proVO.getNombre() + " talla-" + dpVOedit.getTalla()%></option>
                                            <%}%>
                                        </select>
                                    </div>       
                                </div>
                                <div class="">
                                    <input type="date" class="form-control mt-2" name="txtFechaEntregaEdit" id="txtFechaEntregaEdit">
                                </div>
                                <input type="hidden" value="<%=usuVO1.getIdUsuarios()%>" name="txtIdUsuario" id="txtIdUsuarioedit">
                                <input type="hidden" value="2" name="opcionedit" id="opcionedit">
                                <input type="button" id="submitedit" value="Guardar" class="btn boton mt-3">
                                <div id="respuestaEdit"></div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <%--Ventana Eliminar --%>
        <div class="modal fade" id="eliminarOrdenModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header ">
                        <button type="button" class="btn-close botonCerrar" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="col-md-12 mx-auto">
                            <h5 class="text-center text-danger mb-5" id="staticBackdropLabel">¿Esta seguro que desea Eliminar esta Orden?</h5>
                            <table class="table table-hover table-light table-responsive text-center">
                                <thead>
                                    <tr>
                                        <th>Orden</th>
                                        <th>Usuario</th>
                                        <th>Producto</th>
                                        <th>Cantidad Solicitada</th>
                                        <th>Fecha Registro</th>
                                        <th>Fecha Entrega</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td id="idOrdenDel"></td>
                                        <td id="IdUsuarioDel"></td>
                                        <td id="IdDetProDel"></td>
                                        <td id="cantidadDel"></td>
                                        <td id="fechaRegistroDel"></td>
                                        <td id="fechaEntregaDel"></td>
                                    </tr>
                                </tbody>
                            </table>
                            <form>
                                <input type="hidden" value="3" id="opcioneDel">
                                <input type="button" id="submitDel" value="Eliminar" class="btn btn-danger d-inline-block mx-auto ms-5 justify-content-center mt-3"> 
                                <div id="respuestaDel"></div>
                            </form>
                        </div>
                        <div id="responseDel" class="col-md-12 mx-auto justify-content-center">
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <script src="js/JS/ControladorOrdenes.js"></script>

    </body>
</html>
